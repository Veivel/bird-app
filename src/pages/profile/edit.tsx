...
export const fetchCache = 'none', revalidate = 0;
    ...
    return(
        <>
            <div className="mx-auto">
                <h1>Edit Profile</h1>
                { profile ? <UserProfile {...profile} /> : <UserProfile username={''} email={''} avatar={''} created_at={''} />}
                <form encType='multipart/form-data' className='py-8 flex flex-col gap-4'>
                    <Label htmlFor='avatar' color={"black"}>Upload a new profile picture:</Label>
                    <div className='flex'>
                        <FileInput 
                            id="avatar" 
                            name="avatar" 
                        ...
                        />
                        <div className='mx-4'>
                            <Button role={"button"} onClick={handleAvatarSubmit}>Submit</Button>
                        </div>
                    </div>
                    <Label color={"black"}>{"Due to ImageKit's caching, changes will not reflect on existing posts."}</Label>
                </form>
            </div>
        </>
    )
