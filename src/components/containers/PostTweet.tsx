...
export default function PostTweet({
    onNewPost,
}: POST_TWEET_PROPS): JSX.Element {
    const [data, setData] = useState<POST_BODY>({
        text: '',
    ...
    function handleFormSubmit(e: BaseSyntheticEvent) {
        e.preventDefault()
        if (data.text.length < 0 || data.text.length > 320) {
            toast.error('Invalid post content!')
            return
        }
